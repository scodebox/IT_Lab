...
def get_index(pid_q, at_q, bt_q):
    # If there is more than one max burst time. We need to check least arrival time.
    if bt_q.count(max(bt_q)) > 1:
        m = max(bt_q)
        ...
        # Returning the max burst time.
        index = bt_q.index(max(bt_q))
        return pid_q[index]
